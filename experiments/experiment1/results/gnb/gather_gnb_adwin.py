            ...
            for l, distribution in enumerate(distributions):
                for m, flip_y in enumerate(label_noises):
                    ...
                    if spacing == None and drift_type == True:
                        ...
                        print(stream)
                        results = np.load(
                            "experiments/experiment1/results-adwin/gnb/ADWIN/%s" % str(stream).replace('d97','d96') + '.npy'
                        )

                        scores[i, j, k, l, m] = results
scores_metrics_22 = scores
np.save("scores_exp_1_matrix_gnb_adwin", scores_metrics_22)
scores = np.mean(scores_metrics_22, axis=1)
print(scores.shape)
np.save("scores_exp_1_gnb_adwin", scores)
