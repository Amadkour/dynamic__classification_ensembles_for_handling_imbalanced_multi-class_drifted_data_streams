...
def plot_runs(
    ...
    if metrics[i] == "G-mean":
        plt.savefig(
            "experiments/experiment1/results/gnb/ADWIN/plots/scatter/%s_%s_%s.png" % (clfs[j], metrics[i], dependency[k]),
...
def plot_radars(
    ...
    ax.set_yticklabels([])
    plt.savefig(
        "experiments/experiment1/results/gnb/ADWIN/plots/radar/%s_%s.png" % (classifier_name, parameter_name),
