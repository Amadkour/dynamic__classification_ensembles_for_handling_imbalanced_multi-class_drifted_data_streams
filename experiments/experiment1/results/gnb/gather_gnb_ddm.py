...
methods = [
    ...
    "GNB-KNORAU1", "GNB-KNORAU1", "GNB-KNORAU1","GNB-KNORAU1",
    # "SVM-KNORAU1",
                ...
                for m, flip_y in enumerate(label_noises):
                    ...
                    if spacing == None and drift_type == True:
                        ...
                        results = np.load(
                            ...
                            "experiments/experiment1/results-adwin/gnb/DDM/%s" % str(stream).replace('d97','d96') + '.npy'
                        )

                        scores[i, j, k, l, m] = results
scores_metrics_22 = scores
np.save("scores_exp_1_matrix_gnb_ddm", scores_metrics_22)
scores = np.mean(scores_metrics_22, axis=1)
print(scores.shape)
np.save("scores_exp_1_gnb_ddm", scores)
