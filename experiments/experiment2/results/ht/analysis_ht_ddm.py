...
def plot_runs(
    ...
    if metrics[i] == "G-mean":
        # plt.show()
        plt.savefig(
            "experiments/experiment2/results/ht/DDM/plots/scatter/%s_%s_%s.png" % (
...
def plot_radars(
    ...
    # plt.show()
    plt.savefig(
        "experiments/experiment2/results/ht/DDM/plots/radar/%s_%s.png" % (classifier_name, parameter_name),
...
for j, name in enumerate(names):
    print("\n---\n--- %s\n---\n" % (name))
    scores = np.load("experiments/experiment2/results/ht/DDM/%s.npy" % name)
