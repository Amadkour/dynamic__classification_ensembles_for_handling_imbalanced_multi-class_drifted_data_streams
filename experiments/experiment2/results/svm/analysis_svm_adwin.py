...
def plot_runs(
    ...
    if metrics[i] == "G-mean":
        # plt.show()
        plt.savefig(
            "experiments/experiment2/results/svm/ADWIN/plots/scatter/%s_%s_%s.png" % (
...
def plot_radars(
    ...
    # plt.show()
    plt.savefig(
        "experiments/experiment2/results/svm/ADWIN/plots/radar/%s_%s.png" % (classifier_name, parameter_name),
...
for j, name in enumerate(names):
    print("\n---\n--- %s\n---\n" % (name))
    scores = np.load("experiments/experiment2/results/svm/ADWIN/%s.npy" % name)
