...
ls = ["--", "-", ":", "-", ":"]
lw = [1, 1, 1, 1, 1]
names = ["covtypeNorm-1-2vsAll-pruned", ]
...
def plot_runs(
    ...
    for z, (value, label, mean) in enumerate(
        ...
        plt.plot(val%z, label=label, c=colors[z], ls=ls[z], lw=lw[z])
    ...
    if metrics[i] == "G-mean":
        # plt.show()
        plt.savefig(
            "experiments/experiment2/results/svm/DDM/plots/scatter/%s_%s_%s.png" % (
...
def plot_radars(
    ...
    # plt.show()
    plt.savefig(
        "experiments/experiment2/results/svm/DDM/plots/radar/%s_%s.png" % (classifier_name, parameter_name),
...
for j, name in enumerate(names):
    print("\n---\n--- %s\n---\n" % (name))
    scores = np.load("experiments/experiment2/results/svm/DDM/%s.npy" % name)
    ...
    for j, clf in enumerate(clfs):
        ...
        for i, metric in enumerate(metrics):
            ...
            plot_runs(clfs, metrics, [s + 0.05 for s in sub_scores], methods, reduced_scores, name, "")
