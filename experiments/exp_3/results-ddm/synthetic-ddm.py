...
from csm.train_and_test_stratigy import MyTestThenTrain, MyTestThenTrain
...
def worker():
    ...
    for index, classifer_name in enumerate(classifiers_name):
        ...
        np.save("results-ddm/score", eval.scores)
        np.save("results-ddm/score_overlapped", eval.overlappedItems)
        np.save("results-ddm/score_time", eval.execution_time)
