...
from csm.train_and_test_stratigy import MyTestThenTrain, MyTestThenTrainDDM
...
def worker():
    ...
    for index, classifer_name in enumerate(classifiers_name):
        ...
        eval = MyTestThenTrainDDM(metrics=(
            balanced_accuracy_score,
            geometric_mean_score_1,
            f1_score,
            precision,
        ...
        np.save("results/score", eval.scores)
        np.save("results/score_overlapped", eval.overlappedItems)
        np.save("results/score_time", eval.execution_time)
