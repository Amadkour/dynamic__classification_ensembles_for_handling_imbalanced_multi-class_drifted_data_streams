...
for j, name in enumerate(names):
    ...
    reduced_scores = np.mean(scores, axis=1)
    table = []
    header = ["Metric"] + clfs
    # print(reduced_scores2)
    for i, metric in enumerate(metrics):
        # METHOD, CHUNK, Metryka
        selected_scores = np.transpose(reduced_scores)[i]
...
# plt.xlabel('Overlapped synthetic points for Cover Type', fontweight='bold')
